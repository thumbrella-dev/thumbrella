...
fn is_raw_format(ext: &str) -> bool {
    matches!(
        ext,
        "dng" | "cr2" | "nef" | "arw" | "orf" | "rw2" | "pef" | "srw" | "raf" | "3fr" | "fff" | "iiq" | "raw"
    )
}
...
fn extract_raw_preview(reader: Box<dyn ReadSeek + Send>) -> Result<RenderOutput, Box<dyn ReadSeek + Send>> {
    #[allow(unused_mut)]
    ...
    if raw_u16(&header, 2, little) != Some(42) {
        return Err(reader);
    }

    let sub_offs = raw_subifd_offsets(&header, little);
    if sub_offs.is_empty() {
        return Err(reader);
    }
    ...
    if reader.seek(SeekFrom::Start(jpeg_off)).is_err() {
        return Err(reader);
    }
...
fn raw_subifd_offsets(bytes: &[u8], little: bool) -> Vec<u64> {
    let ifd0_off = match raw_u32(bytes, 4, little) {
        Some(v) => v as usize,
        None => return vec![],
    };
    ...
    let count = match raw_u16(bytes, ifd0_off, little) {
        Some(v) => v as usize,
        None => return vec![],
    };
    let mut result = vec![];
    for i in 0..count {
        ...
        let tag = match raw_u16(bytes, e, little) {
            Some(v) => v,
            None => break,
        };
        let ft = match raw_u16(bytes, e + 2, little) {
            Some(v) => v,
            None => break,
        };
        let fc = match raw_u32(bytes, e + 4, little) {
            Some(v) => v as usize,
            None => break,
        };
        let v = match raw_u32(bytes, e + 8, little) {
            Some(v) => v as usize,
            None => break,
        };
        if tag != 0x014A || ft != 4 {
...
fn raw_ifd_jpeg_span(data: &[u8], little: bool) -> Option<(u64, usize)> {
    ...
    for i in 0..count {
        ...
        let tag = match raw_u16(data, e, little) {
            Some(v) => v,
            None => break,
        };
        let v = match raw_u32(data, e + 8, little) {
            Some(v) => v as u64,
            None => break,
        };
        match tag {
        ...
        tokio::task::spawn_blocking(move || {
            match extract_raw_preview(reader) {
                ...
                Err(reader) => {
                    ...
                    crate::avdecode::decode_with_libav(
                        reader,
                        content_length,
                        Some(ext_owned),
                        rotation_hint,
...
async fn render_image_fallback(cook: &mut dyn RenderCook, ext: &str, _content_length: Option<u64>) -> bool {
    ...
    let (result, avio_bytes): (Option<RenderOutput>, u64) = if is_image_crate_format(ext) {
        tokio::task::spawn_blocking(move || {
            (
                collect_and_decode_image_crate(
                    Box::new(Cursor::new(bytes)) as Box<dyn ReadSeek + Send>,
                    cl,
                    &ext_owned,
                ),
                0u64,
...
async fn render_vector(cook: &mut dyn RenderCook, ext: &str, _content_length: Option<u64>) -> bool {
    let reader = match cook.take_reader() {
        Some(r) => r,
        None => {
...
async fn render_video(cook: &mut dyn RenderCook, ext: &str, content_length: Option<u64>) -> bool {
...
fn decode_jpeg_dct(bytes: &[u8], req_w: u16, req_h: u16) -> Option<(DynamicImage, u32, u32, u32)> {
...
fn read_tiff_orientation(tiff: &[u8]) -> Option<u8> {
    ...
    for i in 0..count {
        ...
        let tag = match read_u16_le_be(tiff, e, little) {
            Some(v) => v,
            None => break,
        };
        if tag == 0x0112 {
...
fn isobmff_irot_rotation(data: &[u8]) -> i32 {
    ...
    while pos + 8 <= data.len() {
        ...
        if matches!(
            fcc,
            b"moov" | b"trak" | b"mdia" | b"minf" | b"stbl" | b"meta" | b"iinf" | b"ipco" | b"iprp" | b"dinf"
        ) {
